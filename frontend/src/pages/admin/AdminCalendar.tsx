...
const AdminCalendar: React.FC = () => {
  ...
  const leaveTypeColors = useMemo(() => {
    ...
    const leaveTypes = Array.from(
      new Set(leaveApplications.map(app => app.leaveType?.name))
  ...
  // Get all departments from team members' applications
  const departments = useMemo(() => {
    return ['all', ...Array.from(
      new Set(leaveApplications
        .filter(app => app.employee?.department)
        .map(app => app.employee.department))
    )];
  }, [leaveApplications]);
  ...
  return (
    <div className="p-6">
      ...
      {/* Enhanced Legend with better visual organization */}
